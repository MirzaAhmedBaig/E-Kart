package com.mirza.e_kart.networks

/**
 * Created by avantari on 11/28/17.
 */
class MConfig {
    companion object {
        const val BASE_URL = "https://nagpurfinancepl.com/adminpanel/api/authV2/"
        const val LOGIN = "login"
        const val SIGNUP = "signup"
        const val OTP = "http://sms.webtechsolution.co/vendorsms/pushsms.aspx"
        const val PRODUCTS = "product"
        const val CATEGORIES = "category"
        const val PRODUCT_IMAGES = "images"
        const val CUSTOMER_REQUEST = "customer_request"
        const val REFERRAL_LIST = "referal"
        const val ORDERS = "order"
        const val FEEDBACK = "feedback"
        const val BRANDS = "brands"
        const val PRODUCT_BASE_URL = "https://nagpurfinancepl.com/adminpanel/ghost/storage/app/product/"
        const val IMAGE_BASE_URL = "https://nagpurfinancepl.com/adminpanel/ghost/storage/app/image/"
    }
}

